...
myhostname=$(hostname) #getting dynamic hostname
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') #Getting interface and saving it
lan_address=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}') #getting lan address with ip command
lan_hostname=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}') #getting hostname with hosts file
external_ip=$(curl -s icanhazip.com) #getting external ip from curl which gets output from website
external_name=$(getent hosts $external_ip| awk '{print $2}') #getting external name from hosts file
router_address=$(ip r s default| cut -d ' ' -f 3) #getting router address from ip command
router_hostname=$(getent hosts $router_address|awk '{print $2}') #getting router hostname from hostsfile
